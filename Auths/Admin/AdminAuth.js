...
function AdminAuth(request,response,next){
    ...
    try {
      jwt.verify(accessToken,process.env.JWT_SECRET,function(error,decoded){
        if(error){return response.status(401).json({success:false,message:"Access denied"})}
        if(decoded.role!== "admin"){return response.status(404).json({success:false,message:'Only Admins Are Allowed To Perform This Operation'})}
      })
